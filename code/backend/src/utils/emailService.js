...
// Create transporter with your Gmail credentials (only if not in test)
...
exports.sendPasswordResetEmail = async (recipientEmail, resetUrl, username) => {
  if (isTestEnv) {
    console.log('Skipping password reset email in test environment');
    return true;
  }
  try {
...
exports.sendEmail = async (mailOptions) => {
  if (isTestEnv) {
    console.log('Skipping sendEmail in test environment');
    return true;
  }
  try {
    const info = await transporter.sendMail(mailOptions);
